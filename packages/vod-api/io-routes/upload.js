...
function ioUpload(io) {
  io.of('/upload').on('connection', function (socket) {
    ...
    function calculateProgress() {
      ...
      return Math.min(100, Object.keys(progress).reduce(function(prog, key) {
        if (key === 'encoding') {
    ...
    db.videos.getManagedVideos(socket.user, videoId)
    ...
    subscribeUpload(videoId, function onMessage(message) {
      switch(message.type) {
        case 'step':
          ...
          if (message.file) {
            video.upload.uploadedFiles = video.upload.uploadedFiles.filter(function(file) {
        ...
        case 'progress':
          progress[message.headers.type] = Math.max(progress[message.headers.type] || 0, message.progress);
          socket.emit('progress', {
            progress: calculateProgress(),
            id: videoId,
          });
