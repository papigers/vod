  ...
  playlists.getManagedPlaylists = function(user) {
    return db.knexnest( db.knex
      .select(
        ...
        `videoChannel.name as _videos__channelName`,
      ...
      .leftJoin(`${db.videos.table} as playlistVideo`, `${db.playlistVideos.table}.videoId`, `playlistVideo.id`)
      .leftJoin(`${db.channels.table} as videoChannel`, `playlistVideo.channelId`, `videoChannel.id`)
  ...
  playlists.addVideoToPlaylist = function(user, id, videoId) {
    ...
    .then(function(rows) {
      if (rows.length) {
        ...
        var position = playlist.videos.reduce((max,video) => {return Math.max(max, video.position)},0) + 1;
        return db.knex(db.playlistVideos.table)
        .insert({
          playlistId: id,
          videoId: videoId,
          position: position,
