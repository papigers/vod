...
exports.up = async function(knex) {
  return knex.schema.table(tableName, function(table) {
    ...
    table.enu(
      ...
      { useNative: true, enumName: 'enum_videos_state' },
    ).defaultTo('DRAFT');
  }).then(function() {
    return knex(tableName).update('state', 'PUBLISHED').where('published', true);
  }).then(function() {
    return knex(tableName).update('state', 'DRAFT').where('published', false);
  }).then(function() {
    return knex.schema.table(tableName, function(table) {
...
exports.down = async function(knex) {
  return knex.schema.raw('drop trigger if exists ?? on ??', ['metadata_required_upload_files_trigger', 'videos'])
  .raw('drop function if exists set_required_files_by_metadata() cascade')
  .table(tableName, function(table) {
    table.boolean('published').defaultTo(false);
  })
  .then(function() {
    return knex(tableName).update('published', true).where('state', 'PUBLISHED');
  })
  .then(function() {
    return knex.schema.table(tableName, function(table) {
      table.dropColumns(
