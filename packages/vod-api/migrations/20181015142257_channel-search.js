...
exports.up = function(knex, Promise) {
  ...
  .then(function() {
    return knex('channels').update(
      'tsv',
      knex.raw(
        `setweight(to_tsvector(?, coalesce(??, '')), ?) || setweight(to_tsvector(?, coalesce(??, '')), ?)`,
        ['pg_catalog.english', 'channels.name', 'A', 'pg_catalog.english', 'channels.description', 'C'],
      ),
    ).whereNull('tsv');
  });
};
...
exports.down = function(knex, Promise) {
  return knex.schema.raw('drop trigger if exists ?? on ??;', ['channels_tsvector', 'channels'])
  .table('channels', function(table) {
