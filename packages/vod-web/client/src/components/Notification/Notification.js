...
const NotificationButton = styled(Box).attrs({
  ...
  cursor: pointer;
  background-color: ${({ theme, unread }) => unread ? theme.palette.themeLighterAlt : 'transparent'};

  &:hover {
    background-color: ${({ theme, unread }) => unread ? theme.palette.themeLighter : theme.palette.neutralLighterAlt};
  }
`;
...
class Notification extends Component {
    ...
    switch (type) {
      case 'VIDEO_COMMENT':
        ...
        if (senders.length > 2) {
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו-<NotificationBold>{senders.length - 1}</NotificationBold> אנשים נוספים הגיבו לסרטון {end}
            </span>
          );
        }
          ...
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו<NotificationBold>{senders[1].name}</NotificationBold> הגיבו לסרטון {end}
            </span>
          );
        }
        else {
          text = <span><NotificationBold>{senders[0].name}</NotificationBold> הגיב לסרטון {end}</span>
        }
        break;
      case 'VIDEO_LIKE':
        end = `"${video.name}"`;
        if (senders.length > 2) {
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו-<NotificationBold>{senders.length - 1}</NotificationBold> אנשים נוספים אהבו את הסרטון {end}
            </span>
          );
        }
        else if (senders.length > 1) {
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו<NotificationBold>{senders[1].name}</NotificationBold> אהבו את הסרטון {end}
            </span>
          );
        }
        else {
      ...
      case 'CHANNEL_FOLLOW':
        if (senders.length > 2) {
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו-<NotificationBold>{senders.length - 1}</NotificationBold> אנשים נוספים התחילו לעקוב אחריך
            </span>
          );
        }
        else if (senders.length > 1) {
          text = (
            <span>
              <NotificationBold>{senders[0].name}</NotificationBold> ו<NotificationBold>{senders[1].name}</NotificationBold> התחילו לעקוב אחריך
            </span>
          );
        }
        else {
          text = <span><NotificationBold>{senders[0].name}</NotificationBold> התחיל לעקוב אחריך</span>;
        }
        break;
      default:
        text = type;
  ...
  getNotificationPreview = () => {
    switch (this.props.type) {
      ...
      case 'VIDEO_COMMENT':
        return `${process.env.REACT_APP_STREAMER_HOSTNAME}/${this.props.comment.video.id}/thumbnail.png`;
      case 'CHANNEL_FOLLOW':
      default:
        return null;
    }
