...
class QualityMenuItem extends VjsMenuItem {
  handleClick() {
    ...
    this.options_.qualitySwitchCallback(
      this.options_.id,
      this.options_.trackType
    );
  }
}
...
function qualityPickerPluginDASH() {
  var player = this;
    ...
    for (var i = 0; i < SUPPORTED_TRACKS.length; i++) {
      ...
      if (qualityData[track] && qualityData[track].length > 1) {
        ...
        qualityPickerButton = new QualityPickerButton(player, {
          name,
          qualityList: qualityData[track].map(q => {
            return {
              ...q,
              label: q.label === 'auto' ? 'Auto' : (
                q.id >= 3 ? `${q.label} <sup>HD</sup>` : q.label
              ),
            };
...
registerPlugin("qualityPickerPlugin", qualityPickerPluginDASH);
