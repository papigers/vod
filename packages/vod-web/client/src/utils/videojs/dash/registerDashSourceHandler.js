...
class Html5DashJS {
  constructor(source, tech, options) {
    ...
    this.el_.addEventListener('playing', this.notifyVideoQualities_);
  ...
  getQualityLabel(info) {
    ...
    else if (info.width) return Math.round(info.width * 9 / 16) + 'p';
    else if (info.bitrate) return (info.bitrate / 1000) + 'kbps';
    return 0;
  }

  isQualitySelected(qualityIndex) {
    if (qualityIndex === -1) {
      return this.mediaPlayer_.getAutoSwitchQualityFor('video'); 
    }
    return this.mediaPlayer_.getQualityFor('video').qualityIndex === qualityIndex;
  }
  ...
  notifyVideoQualities_ = () => {
    ...
    if (bitrateInfo) {
      if (bitrateInfo.length > 1) {
        trackList.push({
          id: -1,
          label: 'auto',
          selected: this.isQualitySelected(-1),
        });
      }
      ...
      bitrateInfo.forEach((info) => {
        const quality = {
          ...
          label: this.getQualityLabel(info),
          selected: this.isQualitySelected(info.qualityIndex)
        };
        let found = false;
        for (let i = 0; i < trackList.length && !found; i++) {
          found = quality.label === trackList[i].label;
        }
        if (!found) {
    ...
    this.tech_.trigger('loadedqualitydata', payload);

    this.el_.removeEventListener('playing', this.notifyVideoQualities_);
  }
...
const canHandleSource = function (source) {
  let dashExtRE = /\.mpd/i;
...
const handleSource = function (source, tech, options) {
  return new Html5DashJS(source, tech, options);
}

const registerSourceHandler = function (videojs) {
  const dashSourceHandler = {
    canPlayType,
    canHandleSource,
    handleSource,
  };
  ...
  if (!!window.MediaSource) {
    videojs.getTech('Html5').registerSourceHandler(dashSourceHandler, 0);
  }
...
export default {
  register: registerSourceHandler,
};
