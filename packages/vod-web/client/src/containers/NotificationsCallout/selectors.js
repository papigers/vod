...
const makeSelectNotifications = () =>
  createSelector(selectNotifications, notificationState => notificationState.get('notifications'));

const makeSelectUnreadNotificationCount = () =>
  createSelector(makeSelectNotifications(), notifications => notifications.count(notif => notif.get('unread')));
...
const makeSelectNotificationsError = () =>
  createSelector(selectNotifications, notificationState => notificationState.get('error'));

const makeSelectNotificationsLoading = () =>
  createSelector(selectNotifications, notificationState => notificationState.get('loading'));

const makeSelectUnreadNotifications = () =>
  createSelector(makeSelectNotifications(), notifications => notifications.filter(notif => notif.get('unread')));

const makeSelectUnreadIds = () =>
  createSelector(makeSelectUnreadNotifications(), unreadNotifications => unreadNotifications.reduce((ids, notif) => {
