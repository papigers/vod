...
import { makeSelectNotifications, makeSelectNotificationsError, makeSelectNotificationsLoading } from './selectors';
...
class NotificationsCallout extends Component {
  ...
  render() {
    ...
    const lastNotificationKey = notifications.length ? notifications[notifications.length - 1].groupId : null;
    return (
      <Callout {...calloutProps} onDismiss={this.onDismiss}>
        ...
        <ScrollableContent>
          ...
          {notifications.map(not => <Notification {...not} user={user} onDismiss={this.onDismiss} />)}
          {loading ? (
            <ContentPadding>
