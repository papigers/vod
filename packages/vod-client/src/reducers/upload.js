...
export default function uploadReducer(state = uploadInitialState, action) {
  switch (action.type) {
    case SET_UPLOAD_PROGRESS:
      return state.set(
        'progress',
        Math.min(100, getProgress(state.get('step'), action.progress || state.get('progress'))),
      );
    ...
    case SET_UPLOAD_METADATA:
      return state.set('metadata', state.get('metadata').merge(action.metadata));
    ...
    case SET_UPLOAD_ERROR:
      return state.merge(uploadInitialState).set('error', action.error);
