...
class UploadEdit extends Component {
  ...
  subscribeAndLoadVideoData() {
    ...
    axios.get(`/videos/${this.state.videoId}/thumbnails?count=4`).then(({ data: thumbnails }) => {
      this.setState({ thumbnails });
    });

    // TODO TFIRA
    //let socketEndpoint = (window.apiEndpoint).replace('/api', '');
    this.uploadSocket = io.connect(
      `${window.apiEndpoint}/upload?id=${this.state.videoId}`,
    );
  ...
  onRenderChannelOption = item => {
    ...
    return (
      <DropdownOption>
        <Persona imageUrl={option.data.img} text={option.text} size={PersonaSize.size24} />
      </DropdownOption>
    );
  };
